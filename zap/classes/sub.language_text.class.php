...
class language_textZfSubElement extends zfSubElement {
	...
	function display(&$field_markup,&$subscript_markup) {
		...
		if ($e->populated_value['element_'.$e->id.'_'.($i+2)]==1) {
			$field_markup.="<input id=\"element_{$e->id}_{$i}{$this->ail}\" name=\"element_{$e->id}_{$i}\" class=\"element text\" size=\"{$this->size}\" value=\"\" maxlength=\"{$this->maxlength}\" type=\"text\" {$e->readonly}/>";
		} else {

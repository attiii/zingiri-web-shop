			...
			else
			{
				...
				if($this->getRecordByOrder($sOrderId, $sOrderCode))
				{
					...
					else
					{
						...
						$oPayment = $oMollie->createPayment($sIssuerId, round($this->oRecord['transaction_amount'] * 100), $this->oRecord['transaction_description'], zurl(GatewayCore::getRootUrl() . '?page=pay_ideal&action=return&order_id=' . htmlspecialchars($this->oRecord['order_id']) . '&order_code=' . htmlspecialchars($this->oRecord['order_code'])), zurl(GatewayCore::getRootUrl() . '?page=pay_ideal&action=report&order_id=' . htmlspecialchars($this->oRecord['order_id']) . '&order_code=' . htmlspecialchars($this->oRecord['order_code'])));

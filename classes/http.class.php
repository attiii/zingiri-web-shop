...
class wsHTTPRequest
{
	...
	// constructor
	function wsHTTPRequest($url="",$login=false)
	{
		if (!$url) return;
		$this->login=$login;
		$this->_url = $url;
	...
	function DownloadToString($withHeaders=false,$withCookies=false) {
		...
		session_start();
		if (file_exists(get_option('zing_ws_cache'))) {
			$ckfile='';
			if ($_SESSION['tmpfile']) {
				$ckfile=get_option('zing_ws_cache').$_SESSION['tmpfile'].md5($_SESSION['tmpfile']).'.tmp';
			}
		...
		if (count($post) > 0) curl_setopt($ch, CURLOPT_POSTFIELDS, $post); // add POST fields

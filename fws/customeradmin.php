...
<table border="0">
	<tr>
		<td align="left">
		<form method="POST" action="?page=customeradmin"><strong><?php echo $txt['customeradmin1']; ?></strong><br />
		<input type="text" name="lastname" value="<?php echo $lastname; ?>" size="15"> <input
			type="hidden" name="action" value="<?php echo $action; ?>"
		> <input type="submit" value="<?php echo $txt['customeradmin10']; ?>"></form>
		</td>
	</tr>
</table>
</div>
		...
		if ($customers_per_page > 0 && $num_customers > $customers_per_page) {
			...
			for($i = 0; $i < $num_customers; $i++) {
				...
				if ($page_counter == $customers_per_page) {
					...
					else { echo "<a href=\"".zurl("index.php?page=customeradmin&action=$action&num_page=$num_pages")."\">[$num_pages]</a>"; }
					echo " ";
				}
			}
			// the rest (if any)

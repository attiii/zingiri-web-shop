...
if (LoggedIn() == false) {
	...
	<p><?php echo $txt['checklogin1'] ?></p>
		...
		<div style="text-align: center;"><input type="submit" name="wp-submit" id="wp-submit"
			class="button-primary" value="<?php echo $txt['checklogin4'] ?>" tabindex="100"
		/></div>
		<input type="hidden" name="redirect_to"
			value="<?php zurl(get_option('home').'/index.php?'.$pagetoload,true);?>"
		/> <input type="hidden" name="testcookie" value="1" />
		</form>
<div style="text-align:center"><a
	href="<?php zurl(get_option('siteurl')."/wp-login.php?action=lostpassword",true)?>"
><?php echo $txt['checklogin11'] ?></a></div>
	</div>
...
<div id="wslogin">
	<p><?php echo $txt['checklogin1'] ?></p>
		<?php if (defined("ZING")) { ?>
		<form name="login" method="POST" action="<?php echo zurl('index.php',true);?>"><input
			type="hidden" value="login" name="page"
		> <?php } else { ?>
		<form name="login" method="POST" action="login.php"><?php } ?> <input type="hidden"
			value="<?php echo $pagetoload; ?>" name="pagetoload"
		>
		<table class="borderless" width="100%">
			<tr>
		...
		<div style="text-align: center;"><input type="submit" value="<?php echo $txt['checklogin4'] ?>"
			name="sub"
		></div>
		<div style="text-align:center;"><a href="<?php zurl('?page=login&lostlogin=1',true);?>"><?php echo $txt['checklogin11'] ?></a></div>
		</form>
</div>

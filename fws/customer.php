...
$custForm=appsForm('register','add',"?page=customer&action=add&step=save&register=".intval($_GET['register'])."&pagetoload=".urlencode($pagetoload),true);

if ($action=="add" && $_GET['step']=="save" && $custForm->success && $custForm->showform=="saved") {
	foreach ($zingPrompts->vars as $var) { global $$var; }
	$register=wsSetting('require_registration') ? 1 : intval($_GET['register']);
	...
	if ($register) { //registration
		if (LoggedIn() == false) {
			...
			echo $update_query;
			$update_sql = mysql_query($update_query) or die(mysql_error());
			$update_query = "UPDATE `".$dbtablesprefix."customer` SET `GROUP` = 'GUEST' WHERE `ID` = '".$newcustomerid."'";
			$update_sql = mysql_query($update_query) or die(mysql_error());
